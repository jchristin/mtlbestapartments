	...
	client.run(extraction, function(err, res) {
		if (err) {
			...
			row.append("svg")
				...
				.append("circle")
				.attr("cx", function(d) {
					return moment(d.keen.created_at).diff(timeOrigin, "seconds") * 3;
				})
				.attr("cy", 12)
				.attr("r", 3)
				.attr("fill", function(d) {
					return typeColors[d.type] || "pink";
				})
				.append("svg:title")
				.text(function(d) {
					return d.type + ": " + (typeof d.value === "object" ? JSON.stringify(d.value) : d.value);
				});
		}
	});
};
